...
# msms sims gravel 
#cmdl0='msms -N %s -ms 1064 1 -seed %s  -t 240  -r 320 250000   -I 2 492  572 0  -n 1 1.4  -n 2 4.58 -g 2 192 -m 1 2 0.31  -m 2 1 0.31  -en 0.023 2 0.186100  -em 0.023000 1 2 6  -em 0.023000 2 1  6 -ej 0.051 2 1  -en 0.148 1 0.73100  -SI %s  2  %s  %s  -Smark -Sp 0.5 -Sc 0 2 %s %s  %s > %s/myout.%s.%s.%s.%s.%s.out' % (Ne ,myseed , time, f, f, wAA, wAa, waa, tmp_dir, f*100,s*100,h*100,t/1000,r )
#os.system(cmdl0)
...
cmdl1='./generate_haplotypefiles.py  %s/myout.%s.%s.%s.%s.%s.out ' % (myuniqname) #(tmp_dir, f*100,s*100,h*100,t/1000,r, ) 
#print cmdl1
os.system(cmdl1)
...
# hd rank 
hdcmdl2='./hd4_percentileatscore_msms_sims_singlefile.py  deltadaf_0-1 %s/myout.%s.%s.%s.%s.%s.out.stats >  %s/myout.%s.%s.%s.%s.%s.out.stats.extreme ' % (myuniqname + myuniqname ) #(tmp_dir, f*100,s*100,h*100,t/1000,r) 
os.system(hdcmdl2) 

              
# hd hit 
hdcmdl3='./hd5_and_hit_thresh_msms_sims.py 1000 0.70 %s/myout.%s.%s.%s.%s.%s.out.stats.extreme > %s/myout.%s.%s.%s.%s.%s.out.stats.extreme.hits '  %  (myuniqname + myuniqname ) #(tmp_dir, f*100,s*100,h*100,t/1000,r)
os.system(hdcmdl3)
...
# standardize ihs 
cmdl6='./standardize_ihs.py  ./ihs.tennessen %s/myout.%s.%s.%s.%s.%s.ihs.pop2  > %s/myout.%s.%s.%s.%s.%s.ihs.pop2.stdz' % (myuniqname + myuniqname) #(tmp_dir, f*100,s*100,h*100,t/1000,r, tmp_dir, f*100,s*100,h*100,t/1000,r)
os.system(cmdl6) 
...
# standardize xpehh 
cmdl8='./standardize_xpehh.py  ./xpehh.tennessen %s/myout.%s.%s.%s.%s.%s.xpehh > %s/myout.%s.%s.%s.%s.%s.xpehh.stdz' % (myuniqname + myuniqname)  #(tmp_dir, f*100,s*100,h*100,t/1000,r, tmp_dir, f*100,s*100,h*100,t/1000,r)  
os.system(cmdl8)
